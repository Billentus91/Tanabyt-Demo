# -*- coding: utf-8 -*-
"""Streamlit Volatility Forecasting App
...
def main():
    ...
    with st.spinner(f"Fetching data for {ticker}..."):
        try:
            ...
            # Drop the first 'forecast_days' rows with NaN values
            df = df.iloc[forecast_days:].copy()
        except Exception as e:
    ...
    fig = go.Figure()
    fig.add_trace(go.Scatter(x=df.index, y=garch_volatility_series, mode='lines', name='GARCH Volatility', line=dict(color='green')))

    # Add LSTM predictions (which are a result of the training data)
    df_lstm_predictions = pd.DataFrame(y_pred, index=df.index[30:], columns=['LSTM Volatility'])
